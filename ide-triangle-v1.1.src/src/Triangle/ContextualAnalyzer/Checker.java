...
public final class Checker implements Visitor {
  ...
  public Object visitCallCommand(CallCommand ast, Object o) {
    ...
    if (binding == null){
      ...
      System.err.println("callCommand");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////TODO:
  ...
  public Object visitUnaryExpression(UnaryExpression ast, Object o) {
    ...
    if (binding == null) {
      ...
      System.err.println("unaryExpresion");
      ast.type = StdEnvironment.errorType;
  ...
  public Object visitFuncDeclaration(FuncDeclaration ast, Object o) {
    if (!ast.visited){ //Si fue visitado es porque estuvo dentro de un recursive
  ...
  public Object visitSimpleVname(SimpleVname ast, Object o) {
    ...
    if (binding == null)
      ...
      if (binding instanceof ConstDeclaration) {
        ...
        ast.variable = true;
        //Codigo agregado
      } else if (binding instanceof VarDeclarationEXP){ 
        ast.type = ((VarDeclarationEXP) binding).E.type;
        ast.variable = true;
      } else if (binding instanceof RangeDeclaration){ 
        ast.type = ((RangeDeclaration) binding).E.type;
        ast.variable = false;
      } else if (binding instanceof InDeclaration){ 
        ast.type = ((ArrayTypeDenoter)((InDeclaration) binding).E.type).T;
        ast.variable = false;
  ...
  @Override
    public Object visitForInCommand(ForInCommand ast, Object o) {
       idTable.openScope();
        ...
        TypeDenoter eType = (TypeDenoter) ast.I.E.visit(this, null);
        if (!(eType instanceof ArrayTypeDenoter)){
          reporter.reportError("Expression must be an array.", "", ast.I.E.position);
        }
    ...
    public Object visitForDoCommand(ForDo ast, Object o) {  
        ...
        TypeDenoter eType1 = (TypeDenoter) ast.R.E.visit(this, null);
        TypeDenoter eType2 = (TypeDenoter) ast.E2.visit(this, null);
        if (!eType1.equals(StdEnvironment.integerType))
          reporter.reportError("expected integer at  range", "", ast.R.E.position);
        if (!eType2.equals(StdEnvironment.integerType))
          reporter.reportError("expected integer at  range", "", ast.E2.position);
        if (ast.R.duplicated)
    ...
    public Object visitCaseLiteral(CaseLiteral ast, Object o) {
        ...
        if(r.contains(ast.T.spelling)){reporter.reportError (ast.T.spelling+" repeated case ", "",
                            ast.T.position);}
        r.add(ast.T.spelling);
    ...
    public Object visitCasesElse(CasesElse ast, Object o) {
        TypeDenoter eType = (TypeDenoter) ast.listC.get(0).visit(this, null);
        for (int i=1;i<ast.listC.size();i++) {
            TypeDenoter eTypeAux = (TypeDenoter)ast.listC.get(i).visit(this, null);
            if(eTypeAux != eType){
                reporter.reportError ("wrong type for const actual parameter", "",
                            ast.listC.get(i).position);
            }
    ...
    public Object visitPro_funcs(Pro_funcs ast, Object o) {       
        if (ast.D2 instanceof ProcDeclaration && ((ProcDeclaration)ast.D2).visited){ 
            return null;
        }
        if (ast.D2 instanceof FuncDeclaration && ((FuncDeclaration)ast.D2).visited){ 
            return null;
        }
        if (ast.D1 instanceof Pro_funcs){
            if (ast.D2 instanceof ProcDeclaration){
                ProcDeclaration D =(ProcDeclaration)ast.D2;
                idTable.enter (D.I.spelling, D);
                if (D.duplicated)
                    reporter.reportError ("identifier \"%\" already declared", D.I.spelling, D.position);
                idTable.openScope(); 
                D.FPS.visit(this, null);
                idTable.closeScope();
                D.visited = true;
                ast.D1.visit(this, null);
            }else if (ast.D2 instanceof FuncDeclaration){
                FuncDeclaration D =(FuncDeclaration)ast.D2;
                D.T = (TypeDenoter) (D).T.visit(this, null);
                idTable.enter (D.I.spelling, D);
                if (D.duplicated)
                    reporter.reportError ("identifier \"%\" already declared",(D).I.spelling, D.position);
                idTable.openScope(); 
            ...
            if (ast.D1 instanceof ProcDeclaration){
                ProcDeclaration D =(ProcDeclaration)ast.D1;
                idTable.enter (D.I.spelling, D);
                if (D.duplicated)
                    reporter.reportError ("identifier \"%\" already declared", D.I.spelling, D.position);
                idTable.openScope(); 
            ...
            }else if (ast.D1 instanceof FuncDeclaration){
                FuncDeclaration D =(FuncDeclaration)ast.D1;
                D.T = (TypeDenoter) (D).T.visit(this, null);
                idTable.enter (D.I.spelling, D);
                if (D.duplicated)
                    reporter.reportError ("identifier \"%\" already declared",(D).I.spelling, D.position);
    ...
    @Override
    public Object visitcompoundDeclRecursive(compoundDeclRecursive ast, Object o) {
    ...
    public Object visitcompoundDeclLocal(compoundDeclLocal ast, Object o) {
        idTable.lastT.push(idTable.latest);
        ast.D1.visit(this, null);
        idTable.lastD.push(idTable.latest);
        ast.D2.visit(this, null);
        
        IdEntry temp = idTable.latest;
        IdEntry topValue = idTable.lastD.pop();
        while (temp.previous != topValue){temp = temp.previous;}
            temp.previous = idTable.lastT.pop();
        return null;
    }
    ...
    @Override
    public Object visitRangeDecl(RangeDeclaration ast, Object o) {
        return null;
    }

    @Override
    public Object visitInDeclaration(InDeclaration ast, Object o) {
        return null;
    }
    
    //función para visitar el for until command
    @Override
